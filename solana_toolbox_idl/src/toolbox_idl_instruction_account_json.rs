...
impl ToolboxIdlInstructionAccount {
    pub fn as_json(&self, backward_compatibility: bool) -> Value {
        ...
        let pda = self
            .pda
            .as_ref()
            .map(|pda| pda.as_json(backward_compatibility));
...
impl ToolboxIdlInstructionAccountPda {
    pub fn as_json(&self, backward_compatibility: bool) -> Value {
        json!({
            "seeds": self.seeds.iter().map(|blob| blob.as_json(backward_compatibility)).collect::<Vec<_>>(),
            "program": self.program.as_ref().map(|blob| blob.as_json(backward_compatibility)),
        })
    }
}

impl ToolboxIdlInstructionAccountPdaBlob {
    pub fn as_json(&self, _backward_compatibility: bool) -> Value {
