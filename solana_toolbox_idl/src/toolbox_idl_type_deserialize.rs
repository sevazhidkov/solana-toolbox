...
fn idl_type_deserialize(
    ...
    idl_err(format!("unknown type: {}", idl_type), breadcrumbs)
}
...
fn idl_type_deserialize_defined(
    ...
    let idl_type_name = match idl_type_defined.as_str() {
        ...
        None => {
            let idl_type_defined_object =
                idl_as_object_or_else(idl_type_defined, breadcrumbs)?;
    ...
    let idl_type =
        idl_object_get_key_or_else(idl_types, idl_type_name, breadcrumbs)?;
...
fn idl_type_deserialize_option(
    ...
    let data_flag = idl_u8_from_bytes_at(data, data_offset, breadcrumbs)?;
...
fn idl_type_deserialize_struct(
    ...
    for idl_field in idl_type_fields {
        let idl_field_object = idl_as_object_or_else(idl_field, breadcrumbs)?;
        ...
        let idl_field_type =
            idl_object_get_key_or_else(idl_field_object, "type", breadcrumbs)?;
...
fn idl_type_deserialize_array(
    ...
    if idl_type_array.len() != 2 {
        return idl_err(
            format!("type array is malformed: {:?}", idl_type_array),
            breadcrumbs,
        );
    }
    let idl_item_type = &idl_type_array[0];
    let idl_item_length = idl_as_u128_or_else(&idl_type_array[1], breadcrumbs)?;
    let mut data_size = 0;
...
fn idl_type_deserialize_vec(
    ...
    let data_length = idl_u32_from_bytes_at(data, data_offset, breadcrumbs)?;
...
fn idl_type_deserialize_leaf(
    ...
    if idl_type_str == "u8" {
        let int = idl_u8_from_bytes_at(data, data_offset, breadcrumbs)?;
        return Ok((size_of_val(&int), Value::Number(Number::from(int))));
    }
    if idl_type_str == "i8" {
        let int = idl_i8_from_bytes_at(data, data_offset, breadcrumbs)?;
        return Ok((size_of_val(&int), Value::Number(Number::from(int))));
    }
    if idl_type_str == "u16" {
        let int = idl_u16_from_bytes_at(data, data_offset, breadcrumbs)?;
        return Ok((size_of_val(&int), Value::Number(Number::from(int))));
    }
    if idl_type_str == "i16" {
        let int = idl_i16_from_bytes_at(data, data_offset, breadcrumbs)?;
        return Ok((size_of_val(&int), Value::Number(Number::from(int))));
    }
    if idl_type_str == "u32" {
        let int = idl_u32_from_bytes_at(data, data_offset, breadcrumbs)?;
        return Ok((size_of_val(&int), Value::Number(Number::from(int))));
    }
    if idl_type_str == "i32" {
        let int = idl_i32_from_bytes_at(data, data_offset, breadcrumbs)?;
        return Ok((size_of_val(&int), Value::Number(Number::from(int))));
    }
    if idl_type_str == "u64" {
        let int = idl_u64_from_bytes_at(data, data_offset, breadcrumbs)?;
        return Ok((size_of_val(&int), Value::Number(Number::from(int))));
    }
    if idl_type_str == "i64" {
        let int = idl_i64_from_bytes_at(data, data_offset, breadcrumbs)?;
        return Ok((size_of_val(&int), Value::Number(Number::from(int))));
    }
    if idl_type_str == "u128" {
        let int = idl_u128_from_bytes_at(data, data_offset, breadcrumbs)?;
        return Ok((
            size_of_val(&int),
    ...
    if idl_type_str == "i128" {
        let int = idl_i128_from_bytes_at(data, data_offset, breadcrumbs)?;
        return Ok((
            size_of_val(&int),
    ...
    if idl_type_str == "string" {
        let data_length =
            idl_u32_from_bytes_at(data, data_offset, breadcrumbs)?;
        let mut data_size = size_of_val(&data_length);
    ...
    if idl_type_str == "pubkey" || idl_type_str == "publicKey" {
        let data_pubkey =
            idl_pubkey_from_bytes_at(data, data_offset, breadcrumbs)?;
        let data_size = size_of_val(&data_pubkey);
        return Ok((data_size, Value::String(data_pubkey.to_string())));
    }
    return idl_err(
        format!("type 'string': unknown type descriptor: {}", idl_type_str,),
        breadcrumbs,
    );
}
