...
impl Endpoint {
    pub async fn new_program_test(preloaded_programs: &'static [(Pubkey, &str)]) -> Endpoint {
        let mut program_test = ProgramTest::default();
        program_test.prefer_bpf(true);
        for preloaded_program in preloaded_programs {
            program_test.add_program(preloaded_program.1, preloaded_program.0, None);
        }
    ...
    pub fn new_rpc_client(url: String, commitment_config: CommitmentConfig) -> Endpoint {
